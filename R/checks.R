...
check_soil_table <- function(soil_dt, nutrient) {
  ...
  if (is.null(soil_dt)) {
    ...
    if (nutrient == "nitrogen") {
      tbl_col_names <- names(templates_l$soils_l$nitrogen)
      soil_n_dt     <- soil_dt[, .SD, .SDcols = tbl_col_names]
      ensure_as_template(soil_n_dt, templates_l$soils_l$nitrogen)
    } else {
      if (nutrient == "phosphorus") {
        tbl_col_names <- names(templates_l$soils_l$phosphorus)
        soil_p_dt     <- soil_dt[, .SD, .SDcols = tbl_col_names]
        ensure_as_template(soil_p_dt, templates_l$soils_l$phosphorus)
      } else {
        # potassium
        tbl_col_names <- names(templates_l$soils_l$potassium)
        soil_k_dt     <- soil_dt[, .SD, .SDcols = tbl_col_names]
        ensure_as_template(soil_k_dt, templates_l$soils_l$potassium)
      }
    }
  }
}
...
check_vars <- function(vars, nutrient) {
  ...
  if (is.null(vars)) {
    ...
    # nitrogen
    if (nutrient == "nitrogen") {
      vars_col_names <- names(templates_l$vars_l$nitrogen)
      vars_n_dt      <- vars[, .SD, .SDcols = vars_col_names]
      ensure_as_template(vars_n_dt, templates_l$vars_l$nitrogen)
    } else {
      if (nutrient == "phosphorus") {
        vars_col_names <- names(templates_l$vars_l$phosphorus)
        vars_p_dt      <- vars[, .SD, .SDcols = vars_col_names]
        ensure_as_template(vars_p_dt, templates_l$vars_l$phosphorus)
      } else {
        # potassium"
        vars_col_names <- names(templates_l$vars_l$potassium)
        vars_k_dt      <- vars[, .SD, .SDcols = vars_col_names]
        ensure_as_template(vars_k_dt, templates_l$vars_l$potassium)
      }
    }
  }
}
